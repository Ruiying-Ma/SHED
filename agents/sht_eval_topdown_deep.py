...
def clean_txt(text):
    return white_space_fix(replace_punc(normalize_string(text))).lower()
...
def load_textspan(dataset, qinfo, sht_type):
    ...
    for np in m_np_ts.keys():
        ...
        m_np_header_ts[np]['clean_greptext'] = clean_txt(header + "\n" + m_np_header[np]['text']) # exluding heading
        m_np_header_ts[np]['clean_text'] = clean_txt(header + "\n" + textspan) # excluding heading
    return m_np_header_ts
    
def eval_sht_per_doc_topdown(dataset, qinfo, sht_type):
    file_name = qinfo['file_name']
    ...
    max_precision_list = []
    max_f1_list = []
    sorted_true_np = sorted(true_greptext.keys(), key=lambda x: tuple([int(i) for i in x.split('.')]))
    last_true_np = sorted_true_np[-1]
    for np, true_header_ts in true_greptext.items():
        ...
        for pred_np, pred_header_ts in pred_greptext.items():
            ...
            elif clean_true_header in pred_header_ts['clean_greptext']:
                pred_textspan = pred_header_ts['clean_text']
                pred_metric = eval_textspan(pred_textspan, true_header_ts['clean_text'])
            else:
                continue
        ...
        if last_true_np == np and sht_type in ['llm_txt_sht', 'llm_vision_sht']:
            if metric_info['max_recall_np'] != None:
                continue
...
def eval_sht_per_dataset_top_down(dataset, sht_type):
    ...
    for qinfo in queries[start_id:end_id]:
        avg_recall, avg_precision, avg_f1 = eval_sht_per_doc_topdown(dataset, qinfo, sht_type)
