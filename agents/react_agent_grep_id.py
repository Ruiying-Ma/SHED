...
AGENT_NAME = "react_agent_grep_id"
...
def get_agent_system_message(dataset):
    ...
    "- You can use the tool `grep_search_id(pattern)` to locate relevant content in the document using regular expression matching. The tool searches the entire document and returns the number prefixes of all minimum matching sections. A minimum matching section is defined as a section that contains at least one match to the pattern, while none of its subsections (as defined by the document's table of contents) contain any matches. Use this tool when you want to find relevant sections that contain specific keywords or phrases, especially when you are not sure which sections might be relevant based on the TOC alone.\n\n" \
    "RULES:\n" \
...
@tool(args_schema=grep_search_id_schema)
def grep_search_id(
    pattern: str,
    runtime: ToolRuntime,
) -> str:
    """Searches the document using full regular expression syntax and returns the number prefixes of all minimum matching sections. A minimum matching section is one that contains at least one match to the pattern, while none of its subsections contain any matches."""
    try:
        re.compile(pattern)
...
def run_agent_per_query(dataset, qinfo, model):
    ...
    tools = [read_section, grep_search_id]
    agent = create_agent(
