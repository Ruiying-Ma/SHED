...
from structured_rag.utils import get_nondummy_ancestors, get_successor_head_id, get_grep_successor_head_id
import traceback
import unicodedata
import string
import calendar
...
def get_textspan_clean(nodes: List[Dict], node_id: int, dataset, sht_type):
            ...
            if nodes[ni]['type'] == 'text':
                ...
                if ni == node_id:
                    if dataset in ['contract', 'finance', 'qasper']:
                        ...
                        raise NotImplementedError(f"Heading formatting for dataset {dataset} not implemented yet")
                else:
                    textspan += nodes[ni]['heading'].strip()
...
def orig_toc_textspan_per_doc(dataset, toc_nodes, sht_type):
    ...
    for node_id, level in m_id_level.items():
        if node_id == -1:
            continue
        text_span = get_textspan_clean(toc_nodes, node_id, dataset, sht_type)
...
def merge_toc_textspans(sht_type, m_doc_map_level_textspan, orig_dataset):
    ...
    if sht_type == 'wide':
        ...
        for doc, map_level_ts in m_doc_map_level_textspan.items():
            ...
            map_level_textspan[doc_level] = format_doc_name(doc, orig_dataset)
            map_level_textspan["1"] +=  '\n\n' + format_doc_name(doc, orig_dataset) + "\n\n" +  get_doc_txt(orig_dataset, doc)
        ...
        for doc, map_level_ts in m_doc_map_level_textspan.items():
            doc_level = format_doc_level_str(tuple([1 for _ in range(len(map_level_textspan) + 1)]))
            new_txt = format_doc_name(doc, orig_dataset) + "\n\n" + get_doc_txt(orig_dataset, doc)
            for existing_level in map_level_textspan:
                map_level_textspan[existing_level] += "\n\n" + new_txt
            map_level_textspan[doc_level] = new_txt
            # sorted ts by level
            sorted_level_ts = sorted(map_level_ts.items(), key=lambda x: x[0])
            for level, ts in sorted_level_ts:
                new_level = format_doc_level_str(tuple([1 for _ in range(len(map_level_textspan) + 1)]))
                map_level_textspan[new_level] = ts
        return map_level_textspan
        ...
        for doc, map_level_ts in m_doc_map_level_textspan.items():
            ...
            map_level_textspan[format_doc_level_str(doc_level)] = format_doc_name(doc, orig_dataset) + "\n\n" + get_doc_txt(orig_dataset, doc)
            map_level_textspan["1"] +=  '\n\n' + format_doc_name(doc, orig_dataset) + "\n\n" +  get_doc_txt(orig_dataset, doc)
            # sorted ts by level
            sorted_level_ts = sorted(map_level_ts.items(), key=lambda x: x[0])
            for level, ts in sorted_level_ts:
...
def merge_toc_textspan_per_query(orig_dataset, new_file_name, sht_type, new_file_names, merged_dataset, need_store=True):
    ...
    for fid, file_name in enumerate(new_file_names):
        ...
        map_toc_ts = orig_toc_textspan_per_doc(orig_dataset, toc_nodes, sht_type) # {level: textspan}
