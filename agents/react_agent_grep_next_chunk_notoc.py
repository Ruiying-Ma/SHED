...
AGENT_NAME = "react_agent_grep_next_chunk_notoc"
...
def grep_search_next(
    ...
    """Searches the document using full regular expression syntax and returns matching chunks incrementally across tool calls. 

A matching chunk contains at least one match of the `pattern`, with the match(es) near the center and surrounding context included.
...
If there are no more matching chunks to return for the given `pattern`, the tool returns 'No more matches found.'"""
    ...
    # new pattern
    # return all matching chunks for the new patthern as a list in reading order
    doc_txt = runtime.context['doc_txt']
    ...
    return Command(
        update={
            "messages": [ToolMessage(content=f"{matched_chunks[0]}", tool_call_id=runtime.tool_call_id)],
            "patterns_matches": [[pattern, matched_chunks]], # use reducer
            "patterns_cursor": new_patterns_cursor,
        }
    )
...
def run_agent_per_query(dataset, qinfo, model, sht_type):
    ...
    try:
        final_state: AgentState = agent.invoke(
            ...
            context=DocChunkAgentContext(doc_txt=doc_txt),
