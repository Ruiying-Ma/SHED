...
def get_node_clustering(dataset):
    ...
    llm_txt_folder = os.path.join(root_folder, "llm_txt_sht") 
    os.makedirs(llm_txt_folder, exist_ok=True)
    with open(os.path.join("/home/ruiying/SHTRAG/agents/results/llm_gen_toc_response/gpt-5.4/llm_txt", f"{dataset}.jsonl"), 'r') as file:
    ...
    for raw_md_info in raw_md_list:
        ...
        m_file_toc[file_name] = raw_md_info['message']
    
    parse_rate_list = []

    for file_name, raw_md in m_file_toc.items():
        dst_path = os.path.join(llm_txt_folder, "node_clustering", f"{file_name}.json")
        if os.path.exists(dst_path):
            print(f"Node clustering already exists for {file_name}, skipping...")
            continue
        cluster_list, parse_rate = _get_node_clustering([raw_md], file_name, dataset)
